const charger_IPV4 = "192.168.0.197";//<<<<<<<<<<
// const charger_IPV4 = "192.168.137.60";//<<<<<<<<<<
...
let clients=[];
function reload_all_client(exception=void 0) {
...
app.get("/admin_debug_fetch",async(req,res)=>{console.log("get /admin_debug_fetch :"+req.url);
  ...
  const results=[]
    ...
    rows.forEach((row)=>{
      ...
      Object.keys(client).forEach((key,index)=>{console.log(key,index,);client=client[key]})
      for (key in client) {
...
app.get("/msg/monitor", (req, res)=>{
  ...
  <html>
    <head>
      ...
      <meta charset="utf-8">
      <title>msg monitor</title>
    </head>
    <body>
      <pre>${msg_msgNtime}</pre>
      <div class="json-formatter-container"></div>
    </body>
    <script>
      eventSource = new EventSource("/msg/event");
...
app.get('/', async (req, res) => {const log=false;
  ...
  try {
    ...
    // }
      ...
      res.send([Finish, void 0, void 0, urlparams.get("Parking Space Num")]);
      return
    }
      ...
      console.log(new Date(this_user["start time"]).getTime() + (this_user["charge duration"] * 60 * 1000))
      res.send([InUse, (new Date(this_user["start time"]).getTime() + (this_user["charge duration"] * 60 * 1000)), void 0, urlparams.get("Parking Space Num")]);
      return
    ...
    if (output[0] == FirstTime || output[0] == InQueue) {
      ...
      res.send([output[0], queueNum, charger_free_time, output[3]]);
      console.log('[output[0], queueNum, charger_free_time, output[3]]');
      return
    }
...
app.post("/selected", async (req, resp) => {const log = false;
...
async function call_charger_move_to(spot) {
  ...
  console.log(result)
  console.log(`Moved to spot ${spot}`);
}
let msg_req_interval=()=>{};
async function msg_req(){
  clearInterval(msg_req_interval);
  console.log((`http://${charger_IPV4}//Serial/monitor?host=192.168.0.199:7000`));
  const result = await fetch(`http://${charger_IPV4}//Serial/monitor?host=192.168.0.199:7000`);
  console.log(result);
}
